...
object TestCoreAgent extends TestKit(AkkaTestBasic.system) {
  ...
  val coreAgentBusinessProtocol = new CoreAgentBusinessProtocol(commonParam)
  coreAgentBusinessProtocol.start()
}


class CoreAgentBusinessProtocolSpec extends AsyncFlatSpec with SpecCommon with CoreAgentClient {

  def sendToCoreAgent(msg: Any, f: Array[Byte] => Assertion): Future[Assertion] = {
    val futResp = TestCoreAgent.coreAgentBusinessProtocol.handleMsg(TransportMsg("akka-http-transport", GenericMsg(msg)))
    futResp map { resp =>
